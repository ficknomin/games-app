...
import { FavoriteGame, Game } from "@/lib/types";
  ...
  return (
    <Card onClick={() => router.push(`/games/${game.id}`)} className="group p-0 rounded-sm overflow-hidden bg-card shadow-md hover:shadow-lg transition-shadow cursor-pointer">
      ...
      <CardContent className="px-3 pb-3 flex justify-between">
        <Button variant={"ghost"} className="opacity-100 hover:!bg-transparent group-hover:opacity-100 md:opacity-0 transition-opacity duration-300 cursor-pointer" onClick={(e) => {
          e.stopPropagation();
          toggleFavorite({
            id: game.id,
            name: game.name,
          ...
          {favorite ? <Heart fill={"red"} className="w-5 h-5" /> : <Heart className="w-5 h-5 bg-transparent" />}
        </Button>
